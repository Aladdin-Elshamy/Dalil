...
const Notification: React.FC = () => {
  ...
  const API_TOKEN = "dalail__eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY3YzJlYzYxZDM1MTgwMDMzYzU5Y2VmMiIsImVtYWlsIjoiZGFvdWRtYWhtb3VkMzMxQGdtYWlsLmNvbSIsInJvbGUiOiJBZG1pbiIsInBob25lTnVtYmVyIjoiMDEwOTI3ODM3NzMiLCJhZGRyZXNzIjoiZGFtbmhvdXIiLCJpYXQiOjE3NDA4Mjg5NDZ9.H9NYcvad9wWyJsykbNxjgL4ShUb_5U71yjhe9mfdn78";
  ...
  const fetchReminders = async () => {
    try {
      ...
      const response = await fetch("https://dalail-project-daoud.vercel.app/api/v1/reminder/all", {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          setReminders([]);
          setError("لا توجد تنبيهات أو انتهت صلاحية الجلسة. جرب تسجيل الدخول مرة أخرى.");
          setLoading(false);
          return;
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingReminder) {
        ...
        const response = await fetch(`https://dalail-project-daoud.vercel.app/api/v1/reminder/update/${editingReminder._id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            "token": API_TOKEN
        ...
        // Create new reminder
        const response = await fetch("https://dalail-project-daoud.vercel.app/api/v1/reminder/create", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "token": API_TOKEN
        ...
        if (result.newReminder) {
          ...
          setReminders(prevReminders => [...prevReminders, result.newReminder!]);
        } else {
      ...
      alert(editingReminder ? "فشل في تحديث التنبيه. حاول مرة أخرى." : "فشل في إضافة التنبيه. حاول مرة أخرى.");
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`https://dalail-project-daoud.vercel.app/api/v1/reminder/delete/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "token": API_TOKEN
      ...
      <div className="container mx-auto px-4 py-8 flex flex-col items-center justify-center">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center w-full py-12">
            ...
            <div className="text-gray-500 text-2xl font-bold mb-2">لا توجد تنبيهات بعد!</div>
            <div className="text-gray-400 text-sm">اضغط على "إضافة تنبيه" لإنشاء أول تنبيه لك</div>
          </div>
        ) : (
          <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" dir="rtl">
              ...
              >
                <div>
                  ...
                  <span className="bg-blue-100 text-blue-600 px-3 py-1 rounded-full text-xs font-semibold mb-3 inline-block">
                    {reminder.repetition === 'daily' ? 'يومي' : 
                     reminder.repetition === 'weekly' ? 'أسبوعي' : 
                     reminder.repetition === 'monthly' ? 'شهري' : 'مرة واحدة'}
                  </span>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md relative animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 justify-center pt-4">
                ...
                >
                  {isSubmitting ? 
                    (editingReminder ? "جاري التحديث..." : "جاري الإضافة...") : 
                    (editingReminder ? "تحديث التنبيه" : "إضافة التنبيه")
                  }
                </button>
              </div>
            </form>
